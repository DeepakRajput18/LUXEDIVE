...
// Using the service role key if available to bypass RLS for seeding, otherwise fallback to anon
const supabase = createClient(supabaseUrl, supabaseServiceRoleKey);

// Helper for image URLs matching categories
...
async function seed() {
    ...
    // First retrieve existing cars so we don't duplicate
    const { data: existingCars, error: fetchErr } = await supabase.from('cars').select('model, brand');
    if (fetchErr) {
        console.error("Failed to fetch existing cars: ", fetchErr);
        process.exit(1);
    }

    const existingNames = new Set(existingCars?.map(c => `${c.brand} ${c.model}`));
    ...
    for (const car of NEW_CARS) {
        ...
        // Remove image_url since its now in images array, unless db added it via migration
        // We'll leave it as we also have an image_url column from our migration

        const { error } = await supabase.from('cars').insert(payload);

        if (error) {
            console.error(`Failed to insert ${car.brand} ${car.model}: `, error.message);
        } else {
